...
def retrieve_npr_content(url: str) -> Union[str, None]:
    try:
...
def main_npr_politics_transform(
    ...
    entries: QuerySet = get_entries_above_date(
        LoadedFeedEntry, source_name=source_name, date_dt=max_date
    )
    ...
    entries_df["content"] = entries_df["url"].astype(str).map(retrieve_npr_content)
